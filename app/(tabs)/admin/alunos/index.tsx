...
type A = { _id: string; nome: string; email: string };
type Paged<T> = { items: T[]; total: number; page: number; limit: number };

export default function AlunosList() {
  const [data, setData] = useState<Paged<A>>({ items: [], total: 0, page: 1, limit: 10 });
  ...
  const removeItem = (id: string) => {
    Alert.alert('Excluir', 'Tem certeza?', [
      ...
      { text: 'Excluir', style: 'destructive', onPress: async () => { await api.delete(`/alunos/${id}`); load(data.page); } }
    ]);
  };
  ...
  return (
    <View style={{ flex: 1, padding: 16, gap: 8 }}>
      <Link href="/admin/alunos/new" asChild><TouchableOpacity style={{ backgroundColor: '#0a7', padding: 14, borderRadius: 12 }}><Text style={{ color: 'white', textAlign: 'center', fontWeight: '700' }}>Novo Aluno</Text></TouchableOpacity></Link>
      <FlatList
        data={data.items}
        ...
        renderItem={({ item }) => (
          <View style={{ paddingVertical: 12 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 12, marginTop: 6 }}>
              <TouchableOpacity onPress={() => router.push(`/admin/alunos/${item._id}`)} style={{ padding: 8, backgroundColor: '#08c', borderRadius: 8 }}><Text style={{ color: 'white' }}>Editar</Text></TouchableOpacity>
